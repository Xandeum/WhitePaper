\documentclass[11 pt]{article}   	% use "amsart" instead of "article" for AMSLaTeX format
\usepackage{amsmath}
...
Today, the underperformance of blockchain storage solutions compared to conventional Web2 applications presents a significant obstacle to the broader adoption of Web3.

Xandeum endeavors to propel one of the most consequential societal shifts of the 21st century, boosting the collective self-determination of our society, by facilitating the development and operation of storage-enabled decentralized apps (sedApps).
...
Xandeum narrows the gap between applications designed for the blockchain (Web3) and those designed for traditional Web2 architecture. Web2 applications have access to high-speed, high-capacity centralized storage solutions allowing for dynamic creation, reading, updating, and deletion (CRUD) of data.

Xandeum proposes a blockchain-based CRUD storage layer, which is not only censorship-resistant and tamper-proof but also scalable, efficient, and fully integrated into the smart contract platform.

While existing smart contract platforms, all lacking a scalable, blockchain-based storage layer, can be viewed as a "world CPU", Xandeum adds a "world SSD", completing the tech stack and resulting in a full "world computer" that can run Web3 sedApps while not having to make any major compromises in developer experience compared to web2 apps. 
...
Xandeum's architecture facilitates seamless adoption, allowing developers to smoothly transition to Xandeum's storage solution. Xandeum has achieved this by embracing the Move programming language, allowing the same global storage operators to be used for both account and EGGS storage. This underscores Xandeum's commitment to minimizing friction in the transition from Web2 architecture to Xandeum's Web3 decentralized storage solution.
\end{abstract}
\normalsize
